...
def pm_update_claudemd(project_path: str | None = None) -> dict:
    ...
    v0.4.x の dict response shape を完全保持。v0.6.0 で DeprecationWarning、
...
def update_claudemd_cmd(all_projects):
    """[Legacy] update-rules --target=claude-code と等価。v0.6.0 で deprecation 予定。"""
...
### 6.7 atomic write 共通化 (Amendment A6)
...
- **AGENTS.md backup 非対称**: v0.5.0 では AGENTS.md のみ timestamped backup
  を作成、CLAUDE.md は v0.4.x 互換のため backup 非作成。v0.6.0 で対称化予定
  (PMSERV-058)
- **マーカー規約に従わない外部ツール**: 古い手書き `CLAUDE.md` (マーカー無し)
  はマーカー検知 (§6.4 の signal 2) に拾われない。filesystem 検知 (signal 1)
  でカバーされる前提
- **並行プロセスからの lost update**: 同一 `CLAUDE.md` を 2 つの pm-server
  プロセスが同時に編集した場合の保護は未実装。PMSERV-048 (YAML/file atomic
  write + locking) で対応予定
