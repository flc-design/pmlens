"""Per-project content draft staging store (PMSERV-113, ADR-024).

Append-only staging buffer for publication drafts derived from ``.pm``
by-products (lessons / insights / ADRs / defects). Lives at
``<project>/.pm/drafts.db`` (or the existing legacy ``x_drafts.db``) —
**per-project**, NOT the global
``~/.pm/desktop/desktop.db`` outbox — so accounts/projects never co-mingle.

Safety model (ADR-024): pm-server holds NO publishing credentials and performs NO
network / post action, so "never auto-post" is a *structural* fact, not a
gate. The human reviews redacted drafts and publishes manually, outside the
...
DraftStatus = Literal["draft", "redacted", "rejected", "posted"]
DraftSignalType = Literal["lesson", "insight", "adr", "mistake"]
DraftKind = Literal["single", "thread"]
...
class DraftStoreConflictError(RuntimeError):
    """Both database names exist, so the authoritative store is ambiguous."""

    def __init__(self) -> None:
        super().__init__(
            "Both .pm/drafts.db and .pm/x_drafts.db exist; no draft store was opened. "
            "Stop all clients using this project and back up both databases with their "
            "WAL/SHM files before reconciling them. See docs/content-tool-migration.md."
        )


def default_draft_db_path(pm_path: Path) -> Path:
    """Choose the project store without creating, opening or moving files.

    Reuse an existing legacy database in place, including its WAL/SHM files,
    so older clients keep seeing the same drafts. New projects use drafts.db.
    Never silently choose between two stores: their IDs may overlap and their
    histories may diverge. Even a dangling symlink reserves its database name.
    """
    preferred = Path(pm_path) / "drafts.db"
    legacy = Path(pm_path) / "x_drafts.db"
    ...
    return legacy if legacy_exists else preferred
...
def normalize_source_refs(source_refs: list[str]) -> str:
    ...
    this is the dedupe key ``pm_draft_content`` checks against (cross-check:
...
class DraftStore:
    """Per-project append-only staging store for publication drafts.
    ...
    def mark_posted(self, id: int) -> bool:
        ...
        the human publishes manually; this just records that they did.
...
# Unlike outbox's single-slot module global, content drafts are per-project: one
...
_draft_stores: dict[str, DraftStore] = {}


def get_draft_store(db_path: Path) -> DraftStore:
    """Return the per-db_path DraftStore, creating it on first use.
    ...
    if key not in _draft_stores:
        _draft_stores[key] = DraftStore(Path(db_path))
    return _draft_stores[key]


def clear_draft_store() -> None:
    """Reset the whole factory cache (ALL keys).

    Required by pytest fixtures so tests do not leak draft database handles across
    ...
    """
    for store in _draft_stores.values():
        store.close()
    _draft_stores.clear()
